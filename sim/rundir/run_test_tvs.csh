  ...
  if("$cmd_option" == "-func_cov") then
     if("$multims" == "-multims") then
      ...
      vlog -sv -permissiive -novopt +define+QUESTA_SIM +define+UVM_NO_DPI +define+QUESTA \
        +incdir+$UVM_HOME/ $UVM_HOME/uvm_pkg.sv $UVM_HOME/dpi/uvm_dpi.cc $extdef \
          -f ../compile/comp_list.fl | tee -a $testcase_name.log 
	...
	$tcl_opt \
     ...
     if("$multims" == "-multims") then
       ...
       +define+UVM_NO_DPI +define+QUESTA +incdir+$UVM_HOME $UVM_HOME/uvm_pkg.sv $UVM_HOME/dpi/uvm_dpi.cc $extdef \
    ...
    if("$multims" == "-multims") then
      ...
      vlib work
      vlog -sv -permissive -novopt +define+UVM_NO_DPI +define+QUESTA +incdir+$UVM_HOME $UVM_HOME/uvm_pkg.sv $UVM_HOME/dpi/uvm_dpi.cc $extdef \
          -f ../compile/comp_list.fl | tee $testcase_name.log
      vsim -debugDB -c -permissive  $top_module_name \
          +UVM_TESTNAME=$testcase_name \
          +UVM_VERBOSITY=$verbosity  \
	   $tcl_opt \
          -do "$dump_questa; run -a; quit -f" -sv_seed $user_seed | tee -a $testcase_name.log 
     else if("$cov_option" == "-code") then
      vlib work
      vlog -sv  -novopt +cover   \
	  +define+QUESTA  $extdef \
          -f ../compile/comp_list.fl | tee $testcase_name.log
#      qverilog -sv -novopt +cover +define+QUESTA $extdef \
#          -f ../compile/comp_list.fl \
#          -R -coverage +UVM_TESTNAME=$testcase_name +UVM_VERBOSITY=$verbosity  \
#          $tcl_opt \
#          -do "$dump_questa; coverage save -codeAll -cvg -onexit $testcase_name.ucdb; run -a; quit -f" -sv_seed $user_seed \
#         | tee $testcase_name.log
      vsim -debugDB -c -permissive  $top_module_name \
          +UVM_TESTNAME=$testcase_name \
          +UVM_VERBOSITY=$verbosity  \
	   $tcl_opt -coverage $top_module_name  +UVM_TESTNAME=$testcase_name \
          +UVM_VERBOSITY=$verbosity  \
	   $tcl_opt \
          -do "$dump_questa;coverage save -codeAll -cvg -onexit $testcase_name.ucdb;  run -all; exit" \
          -sv_seed $user_seed \
      ...
      qverilog -sv -novopt  +define+QUESTA $extdef \
          -f ../compile/comp_list.fl \
          -R  +UVM_TESTNAME=$testcase_name +UVM_VERBOSITY=$verbosity  \
          $tcl_opt \
          -do "$dump_questa;  run -a; quit -f" -sv_seed $user_seed \
         | tee $testcase_name.log
    ...
    if("$multims" == "-multims") then
      ...
      vlog -sv -permissive -novopt +define+QUESTA +define+QUESTA_SIM \
          -f ../compile/comp_list.fl $extdef  | tee $testcase_name.log 
...
if ! (-e ./logs/$testcase_name) then
  ...
  vsim -view ./logs/$testcase_name/$testcase_name.wlf
